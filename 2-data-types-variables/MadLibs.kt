...
fun main () {
    ...
    println("Adjective for the nature of the trip?")
    var adjective = readLine()
    println("What is the destination city?")
    var city = readLine()
    println("What's the name of the band?")
    var bandName = readLine()
    println("What work did you and $friendsName do to save money?")
    var work = readLine()
    println("Write the distance in km between 400 and 600, (integer numbers only):")
    distance = readLine()!!.toInt()

    var totalForGas = (costOfGas / mileage) * distance
    totalForGas = Math.round(totalForGas * 100) / 100.00
    println("Fuel Cost: $totalForGas")
